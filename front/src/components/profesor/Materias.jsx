...
export default function Materias({ token, idP }) {
    ...
    useEffect(() => {
        try {
            async function obtenerMaterias() {
                const respuesta = await axios.get(`api/materias/profesor/${idP}`,
                    {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Hubo un error al obtener las materias.',
                footer: error
            });
        }
    }, [token, idP]);
    ...
    const ver = id => {
        history.push(`/materia/${id}`);
    }
    const editar = id => {
        history.push(`/editarMateria/${id}`);
    }
    const eliminar = async (id, activo) => {
        try {
            const respuesta = await axios.patch(`/api/materias/${id}`, { activo },
                {
                    headers: { 'x-access-token': token }
                })
            if (respuesta.data.mensaje === 'Eliminado') {
                setMaterias(materias.filter(item => {
                    if (item._id === id) {
        ...
        <div className="container mt-5">
            <MaterialTable
                ...
                actions={[
                    {
                        ...
                        tooltip: '',
                        position: 'toolbar',
                        onClick: () => { history.push("/nuevaMateria") }
                    }
                ]}
                options={{
                ...
                localization={{
                    ...
                    body: {
                        emptyDataSourceMessage: 'Sin materias'
                    }
                }}
            />
        </div>
