...
export default function EditarReactivo({ token, idP }) {
    let ponderaciones = [];
    const { id } = useParams();
    ...
    const { pregunta, tema, materia, estadoCalibrado, opcionCorrecta, opcionIncorrecta1, opcionIncorrecta2, opcionIncorrecta3,
        ponderacion, opcionCorrectaText, opcionIncorrecta1Text, opcionIncorrecta2Text, opcionIncorrecta3Text } = datos;
    ...
    useEffect(() => {
        try {
            ...
            async function obtenerReactivo() {
                const respuesta = await axios.get(`/api/reactivos/${id}`,
                    {
                        headers: { 'x-access-token': token }
                    });
                setDatos(respuesta.data)
                setFormulas(respuesta.data.formula)
            }
            obtenerTema();
            obtenerMateria();
            obtenerReactivo();
    ...
    const enviar = async e => {
        ...
        if (pregunta === "" || tema === "" || materia === "" ||
            ...
            try {
                const respuesta = await axios.put(`/api/reactivos/${id}`, datos,
                    {
                        headers: { 'x-access-token': token }
                    });
                if (respuesta.data.mensaje === 'Editado') {
                    Swal.fire(
                        'Éxito',
                        'El reactivo se modifico correctamente',
                        'success'
                    );
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: 'Hubo un error al modificar el reactivo.',
    ...
    return (
        <div className="container p-5">
            <h1 className="text-primary text-center">Editar Reactivo</h1>
